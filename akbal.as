...
function akbitchEncounter():void {
	...
	if(player.cor < 40 || player.lust < 33) akbitchNoThnx(false);
	//{corruption > 40}
	else {
...
function takeAdvantageOfAkbitch():void {
	...
	//AMB Strength Scene
	//70+
	if(player.str >= 70) addButton(1,"Strong",akbitchHighStrengthVariant);
	//AMB Speed Scene
	//70
	if(player.spe >= 70) addButton(2,"Fast",akbalBitchSpeed);
	//AMB Toughness Scene
	//70
	if(player.tou >= 70) addButton(3,"Toughness",akbitchToughness);
}
...
function basicAkbitchScene():void {
	...
	if(flags[AKBAL_TIMES_BITCHED] == 1) outputText("The smile on your [face] spreads even wider as the unexpected sound tells you you’ve turned this demonic sexual predator into your own personal slut.  As if to confirm this, h");
	else outputText("H");
...
function buttFuckbuttFuckbuttFuckAkbal():void {
	clearOutput();
	//[if (hasCock = true)]
	if(player.hasCock()) outputText("You widen your stance as you sink into the demon’s moist depths with a grin.");
	if(player.cockTotal() > 1) outputText("  You widen your stance and look down, choosing the biggest among [eachCock].");
...
function topAkbitchFromBottomDuex():void {
	...
	if(player.hasVagina()) cuntChange(16, true, true, false);
	else buttChange(16, true, true, false);
	...
	outputText("\n\nYou look back at your new bitch with a grin while he regains his senses.  As you leave the forest, you hear a promise from Akbal’s chorus of voices, \"<i>You will regret this... Champion.</i>\"");
	stats(0,0,0,0,0,0,-100,3);
	if(player.hasVagina()) player.knockUp(1,432,101);
	doNext(13);
}
...
function akbitchToughness():void {
	...
	//{female/genderless}
		...
		if(player.thickness <= 33) outputText("slim");
		else if(player.thickness <= 66) outputText("muscular");
